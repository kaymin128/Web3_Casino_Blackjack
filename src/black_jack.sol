...
contract BlackJack {
    ...
    mapping(uint256 => player) private player_info; // player_info[key]라는 구조체 변수 생성
    address public dealer;
    uint256 public total_bet;
    bool all_stay;
    uint256 private deal_num;
    uint256[] winner;
    program_state public p_state;
    bool private stopped; // Emergency Stop: 계약의 실행을 중지할 수 있는 플래그
    mapping(address => uint256) public pending_withdrawals; // Pull over Push: 보상을 직접 인출할 수 있도록 보상 대기열을 관리
    ...
    modifier only_player(uint256 key) {
    ...
    function initialize() external {
        // 카드를 값을 정해 주고 초기화 해줌(이전 게임 정보 삭제)
        dealer = address(0);
    ...
    function register(uint256 betting)
        external
        payable
        stop_in_emergency
        in_state(program_state.new_born)
        returns (uint256 key)
    {
        // 사용자가 플레이어로 등록하고 베팅
    ...
    function dealer_register(uint256 betting)
        external
        payable
        in_state(program_state.players_registered)
        stop_in_emergency
    {
        require(betting > 1 ether, "not enough betting");
        total_bet += betting;
    ...
    function hit(uint256 key)
        external
        stop_in_emergency
        only_player(key)
        in_state(program_state.dealer_registered)
        returns (uint256 num)
    {
        player_info[key].num += cards[uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 13];
    ...
    function stay(uint256 key) external only_player(key) stop_in_emergency in_state(program_state.dealer_registered) {
    ...
    function dealing_time() public in_state(program_state.people_choosed) stop_in_emergency {
        while (true) {
            deal_num += cards[uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 13];
            if (deal_num >= 17) {
                break;
            }
        } // deal_num까지 구하면
    ...
    function finish_game() public in_state(program_state.dealer_choosed) stop_in_emergency {
        ...
        if (winner.length > 0) {
            // winner인 player가 존재할 때
            uint256 reward = total_bet / winner.length;
    ...
    function withdraw() external payable {
        // Pull over Push: 사용자가 원할때 직접 보상을 인출
        uint256 amount = pending_withdrawals[msg.sender];
    ...
    function failed(uint256 key) private {
        player_info[key].bet = 0;
        player_info[key].current_state = state.dead;
    }

    function emergency() public only_owner {
        stopped = true;
    }

    function finish_emergency() public only_owner {
        stopped = false;
    }
